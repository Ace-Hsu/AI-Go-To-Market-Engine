#!/usr/bin/env python3
"""
Gap Analysis Evaluation Tool

This script provides a GUI for evaluating gap analysis reports from 3_unlabeled/
...
class GapAnalysisEvaluator:
    def __init__(self):
        ...
        # Scoring criteria with weights (adapted for gap analysis)
        self.scoring_criteria = {
            "framework_adherence": {"weight": 0.2, "description": "Follows 4-step methodology and required format"},
            "quantitative_evidence": {"weight": 0.2, "description": "Uses actual percentages and data-driven findings"},
            "business_relevance": {"weight": 0.2, "description": "Focuses on gaps that impact revenue/growth"},
            "gap_clarity": {"weight": 0.2, "description": "Clear, specific gap statements using template"},
            "strategic_prioritization": {"weight": 0.2, "description": "Appropriate ranking by business impact"}
        }
        
        # Improvement tags (adapted for gap analysis)
        self.improvement_tags = [
            "quantitative_analysis", "business_impact", "framework_methodology", 
            "gap_statement_clarity", "strategic_prioritization", "actionable_insights"
        ]
    ...
    def setup_ui(self):
        self.root = tk.Tk()
        self.root.title("Gap Analysis Evaluator")
        ...
        main_notebook.add(content_frame, text="Gap Analysis Content")
        
        # Text area for displaying gap analysis
    ...
    def load_unlabeled_files(self):
        ...
        try:
            ...
            if self.current_project:
                ...
                # Look for gap_analysis_*.md files in project directory
                md_files = list(project_unlabeled_dir.glob("gap_analysis_*.md"))
                ...
                if not md_files:
                    ...
                    if root_unlabeled_dir.exists():
                        md_files = list(root_unlabeled_dir.glob("gap_analysis_*.md"))
            else:
                ...
                # Get all gap_analysis_*.md files
                md_files = list(self.unlabeled_dir.glob("gap_analysis_*.md"))
            
            if not md_files:
                messagebox.showwarning("Warning", f"No gap_analysis_*.md files found")
                return
            ...
            if self.gap_reports:
                self.current_index = 0
                self.update_display()
                messagebox.showinfo("Success", f"Loaded {len(self.gap_reports)} gap analysis files")
            else:
    ...
    def update_display(self):
        """Update the display with current gap analysis"""
        if not self.gap_reports:
            self.text_display.delete(1.0, tk.END)
            self.text_display.insert(tk.END, "No gap analysis reports loaded")
        ...
        current = self.gap_reports[self.current_index]
        ...
        # Update status
        self.status_var.set(f"File {self.current_index + 1} of {len(self.gap_reports)}")
    ...
    def save_current_evaluation(self):
        ...
        evaluation_data = {
            ...
            "gap_analysis_content": {
                "generated_report": current['content']
            },
            "system_learning": {
    ...
    def next_item(self):
        """Move to next gap analysis"""
        if not self.gap_reports:
            return
        
        if self.current_index < len(self.gap_reports) - 1:
            self.current_index += 1
            self.update_display()
    
    def prev_item(self):
        """Move to previous gap analysis"""
        if not self.gap_reports:
            return
