#!/usr/bin/env python3
"""
Website Copy Evaluation Tool

This script provides a GUI for evaluating website copy from 3_unlabeled/
...
class WebsiteCopyEvaluator:
    def __init__(self):
        ...
        # Scoring criteria with weights (adapted for website copy)
        self.scoring_criteria = {
            "strategic_logic_quality": {"weight": 0.3, "description": "Convincing reasoning for homepage narrative flow"},
            "customer_psychology_accuracy": {"weight": 0.25, "description": "Deep understanding of target audience patterns"},
            "content_module_effectiveness": {"weight": 0.2, "description": "Quality and relevance of 7 content modules"},
            "narrative_flow": {"weight": 0.15, "description": "Logical progression from empathy to conversion"},
            "business_impact": {"weight": 0.1, "description": "Focus on revenue/conversion optimization"}
        }
        
        # Improvement tags (adapted for website copy)
        self.improvement_tags = [
            "logic_explanation", "customer_analysis", "content_prioritization", 
            "emotional_journey", "conversion_optimization", "industry_specificity"
        ]
    ...
    def setup_ui(self):
        self.root = tk.Tk()
        self.root.title("Website Copy Evaluator")
        ...
        main_notebook.add(content_frame, text="Website Copy Content")
        
        # Text area for displaying website copy
    ...
    def load_unlabeled_files(self):
        ...
        try:
            ...
            if self.current_project:
                ...
                # Look for website_copy_*.md files in project directory
                md_files = list(project_unlabeled_dir.glob("website_copy_*.md"))
                ...
                if not md_files:
                    ...
                    if root_unlabeled_dir.exists():
                        md_files = list(root_unlabeled_dir.glob("website_copy_*.md"))
            else:
                ...
                # Get all website_copy_*.md files
                md_files = list(self.unlabeled_dir.glob("website_copy_*.md"))
            
            if not md_files:
                messagebox.showwarning("Warning", f"No website_copy_*.md files found")
                return
            ...
            if self.website_copies:
                self.current_index = 0
                self.update_display()
                messagebox.showinfo("Success", f"Loaded {len(self.website_copies)} website copy files")
            else:
    ...
    def update_display(self):
        """Update the display with current website copy"""
        if not self.website_copies:
            self.text_display.delete(1.0, tk.END)
            self.text_display.insert(tk.END, "No website copies loaded")
        ...
        # Get current website copy
        current = self.website_copies[self.current_index]
        ...
        # Update status
        self.status_var.set(f"File {self.current_index + 1} of {len(self.website_copies)}")
    ...
    def save_current_evaluation(self):
        ...
        evaluation_data = {
            ...
            "website_copy_content": {
                "generated_copy": current['content']
            },
            "system_learning": {
    ...
    def next_item(self):
        """Move to next website copy"""
        if not self.website_copies:
            return
        
        if self.current_index < len(self.website_copies) - 1:
            self.current_index += 1
            self.update_display()
    
    def prev_item(self):
        """Move to previous website copy"""
        if not self.website_copies:
            return
...
if __name__ == "__main__":
    evaluator = WebsiteCopyEvaluator()