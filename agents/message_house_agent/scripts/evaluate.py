#!/usr/bin/env python3
"""
Message House Evaluation Tool

This script provides a GUI for evaluating message houses from 3_unlabeled/
...
class MessageHouseEvaluator:
    def __init__(self):
        ...
        self.scoring_criteria = {
            "strategic_depth": {"weight": 0.2, "description": "Depth of strategic thinking, contrarian positioning"},
            "internal_consistency": {"weight": 0.2, "description": "Alignment between roof, pillars, and foundation"},
            "proof_point_quality": {"weight": 0.2, "description": "Strength and credibility of supporting evidence"},
            "customer_psychology": {"weight": 0.2, "description": "Understanding of target audience pain points"},
            "competitive_intelligence": {"weight": 0.2, "description": "Understanding of competitive landscape"}
        }
        
        # Improvement tags
        self.improvement_tags = [
            "clarity", "strategic_focus", "differentiation", 
            "actionability", "emotional_resonance", "proof_strength"
        ]
    ...
    def setup_ui(self):
        self.root = tk.Tk()
        self.root.title("Message House Evaluator")
        ...
        main_notebook.add(content_frame, text="Message House Content")
        
        # Text area for displaying message house
    ...
    def load_unlabeled_files(self):
        """Load all .md files from 3_unlabeled/ directory"""
        try:
            md_files = []
            ...
            if self.current_project:
                ...
                md_files = list(self.unlabeled_dir.glob("messagehouse_*.md"))
                ...
                if not md_files:
                    ...
                    if root_unlabeled_dir.exists():
                        md_files = list(root_unlabeled_dir.glob("messagehouse_*.md"))
            else:
            ...
            if not md_files:
                messagebox.showwarning("Warning", f"No message house .md files found in {self.unlabeled_dir}")
                return
            ...
            if self.message_houses:
                self.current_index = 0
                self.update_display()
                messagebox.showinfo("Success", f"Loaded {len(self.message_houses)} message house files")
            else:
    ...
    def update_display(self):
        """Update the display with current message house"""
        if not self.message_houses:
            self.text_display.delete(1.0, tk.END)
            self.text_display.insert(tk.END, "No message houses loaded")
        ...
        # Get current message house
        current = self.message_houses[self.current_index]
        ...
        # Update status
        self.status_var.set(f"File {self.current_index + 1} of {len(self.message_houses)}")
    ...
    def next_item(self):
        """Move to next message house"""
        if not self.message_houses:
            return
        
        if self.current_index < len(self.message_houses) - 1:
            self.current_index += 1
            self.update_display()
    
    def prev_item(self):
        """Move to previous message house"""
        if not self.message_houses:
            return
...
if __name__ == "__main__":
    evaluator = MessageHouseEvaluator()